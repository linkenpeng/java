...
public class GenSqlTest {
	...
	public static  void genSql() {
		for(int i = 0; i < 64; i++) {
			...
			String sql = "ALTER TABLE `user_cart_new_"+i+"` ADD COLUMN `activity_key` varchar(16) NULL DEFAULT '' COMMENT '活动key' AFTER `activity_level`,ADD COLUMN `activity_rule_code` varchar(32) NULL DEFAULT '' COMMENT '活动的规则类型编码' AFTER `activity_key`;";
	...
	public static  void delSql() {
		for(int i = 0; i < 64; i++) {
			String sql = "update user_cart_new_" + i+ " set delete_mark=1 WHERE key_sales is null; ";
			System.out.println(sql);
		}
	}
